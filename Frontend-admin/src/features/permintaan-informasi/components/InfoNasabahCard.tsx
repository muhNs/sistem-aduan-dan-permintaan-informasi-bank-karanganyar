...
import type { InfoRequestDetail } from '../types/information.types';

export default function InfoNasabahCard({ item }: { item: InfoRequestDetail }) {
  return (
    <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
      ...
      <div className="space-y-5">
        ...
        <div className="flex items-start gap-4">
          ...
          <div>
            <p className="text-xs text-gray-400 mb-0.5">No. Telepon / WhatsApp</p>
            <p className="text-base text-gray-700">{item.nasabah.phone}</p>
          </div>
        </div>

        {item.nasabah.address && (
          <div className="flex items-start gap-4">
            <div className="w-10 h-10 rounded-xl bg-gray-100 flex items-center justify-center shrink-0">
